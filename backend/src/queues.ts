...
import { handleRetryTemplate } from "./controllers/TemplateController";
import { handleLastTemplate } from "./controllers/TemplateController";
...
async function handleRemoveTemplateAutomatic() {
  const job = new CronJob('*/5 * * * *', async () => {
    const contacts = await Contact.findAll();
    const dateNow = new Date();
    const millisecondsIn24Hours = 60000; // 24 * 60 * 60 * 1000;

    contacts.map(async (contact) => {
      const templateDate = contact.templateUpdate;

      try {
        if (templateDate) {
          const dataInterval = dateNow.getTime() - templateDate.getTime();
          if (dataInterval >= millisecondsIn24Hours) {
            const updateContact = await Contact.update(
              { templateUpdate: null },
              { where: { id: contact.id } }
            );
            console.log(
              `Template Update do contato ${contact.id}, novo valor: ${updateContact}`
            );
            ...
            for (const ticket of tickets) {
              ...
              console.log(`Ticket ${ticket.id} atualizado para os valores padrão.`);
            }
          } else {
            return;
          }
        } else {
          console.log("templateUpdate: ", templateDate);
          return;
        }
      } catch (e: any) {
        Sentry.captureException(e);
        logger.error("Erro ao remover template automaticamente: ", e.message);
        throw e;
      }
    });
  });
...
async function handleRetryTemplateAutomatic() {
  const job = new CronJob('* * * * *', async () => {
    try {
      ...
      for (const ticket of tickets) {
        try {
          if (!ticket.contact) {
            logger.error(`Contato não encontrado para o ticket ${ticket.id}`);
            continue; 
          }

          await handleRetryTemplate(ticket);
        } catch (error) {
      ...
      Sentry.captureException(error);
      logger.error(`Erro na função handleRetryTemplateAutomatic: ${error.message}`);
    }
  });
...
async function handleLastTemplateAutomatic() {
  const job = new CronJob('* * * * *', async () => {
    try {
      const tickets = await Ticket.findAll({
        where: {
      ...
      for (const ticket of tickets) {
        try {
          ...
          await handleLastTemplate(ticket);
        } catch (error) {
          Sentry.captureException(error);
          logger.error(`Erro ao processar ticket ${ticket.id}: ${error.message}`);
        }
      ...
      Sentry.captureException(error);
      logger.error(`Erro na função handleLastTemplateAutomatic: ${error.message}`);
    }
  });
...
function getCampaignValidMessages(campaign) {
  ...
  if (campaign.template && campaign.template.mainTemplate) {
    messages.push(campaign.template.mainTemplate);
  } else {
...
async function handlePrepareContact(job) {
  try {
    ...
    if(!templateId){
      ...
      const message = template.mainTemplate;
      campaignShipping.message = `\u200c${message}`;
    }
...
handleRemoveTemplateAutomatic()
handleRetryTemplateAutomatic()
handleLastTemplateAutomatic()
handleInvoiceCreate()

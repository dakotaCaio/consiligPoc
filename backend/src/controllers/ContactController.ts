...
interface IndexQuery {
  ...
  page?: number;
  limit?: number;
  isReport?: boolean;  // Adicionando o parâmetro isReport
}
...
export const index = async (req: Request, res: Response): Promise<Response> => {
  const { name, number, page = 1, limit = 10, isReport = false } = req.query as IndexQuery;
  const { companyId } = req.user;

  try {
    const { contacts, totalContacts } = await ListContactsService({
    ...
    return res.json({ contacts, totalContacts });
  } catch (err) {
...
export const list = async (req: Request, res: Response): Promise<Response> => {
  const { name, number, page = 1, limit = 10, isReport = false } = req.query;

  try {
    const { contacts, totalContacts } = await SimpleListService({
      name: name as string | undefined,
      number: number as string | undefined,
      page: Number(page),
      limit: limit ? Number(limit) : undefined, 

...
import FindAllCompanyIdService from "../services/CompanyService/FindAllCompanyIdService";
...
export const index = async (req: Request, res: Response): Promise<Response> => {
  ...
  const userCompanyId = req.user?.companyId;  // Pegando o ID da empresa do usuário logado

  // Se estamos no endpoint /companies, queremos trazer apenas a empresa do usuário
  const { companies, count, hasMore } = await ListCompaniesService({
    searchParam,
    pageNumber,
    userCompanyId,  // Passando o ID da empresa para o serviço
  });
...
export const listCompanyId = async (req: Request, res: Response): Promise<Response> => {
  const { companyId } = req.params;
  
  const companies = await FindAllCompanyIdService(Number(companyId), req.user);

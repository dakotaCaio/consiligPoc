...
import { head, isNil, update } from "lodash";
    ...
    if (prompt.voice === "texto") {
      ...
      convertTextToSpeechAndSaveToFile(
        ...
        try {
          ...
          throw new Error(`Erro para responder com audio: ${error}`);
        }
      });
    }
...
const verifyCampaignMessageAndCloseTicket = async (
  ...
  const tag4 = await Tag.findByPk(250);
  if (!tag4) {
    console.log("Tag com ID 4 não encontrada!");
  }
  ...
  if (message.key.fromMe && isCampaign) {
    ...
    const template = templateData.find(c => cleanString(c.mainTemplate) === cleanString(body));


    if (template) {
      const mainTemplate = template.mainTemplate;
      ...
      const ticket: any = await Ticket.findByPk(messageRecord.ticketId);

      if (!ticket) {
        console.log("Ticket com ID não encontrado!");
        return;
      }

      const hasTemplateId = !!template.id;
      const updatedTicket = await ticket.update(
        { 
          status: "closed", 
          cpnTickets: true, 
          templateId: template.id,
          mainTemplate: hasTemplateId, 
          cpc: hasTemplateId,
          dateTemplate: new Date()
        }
      );
      ...
      if (contact) {
        await contact.update({ templateUpdate: Date.now() });
      } else {
        console.log("Contato não encontrado para atualização!");
      }

      await updatedTicket.addTag(tag4);
      ...
      if (!messageRecord) {
        console.log("Message com ID não encontrado!");
        return;
      }
    ...
    console.log("Mensagem não é de campanha ou não foi enviada por você.");
  }
};

...
const UpdateTicketService = async ({
  ...
  try {
    const { status, templateId } = ticketData;
    ...
    if (status !== undefined && ["closed"].indexOf(status) > -1) {
      ...
      console.log("ticketData no UpdateTicketService:", ticketData);
    ...
    if (status !== undefined && ["pending"].indexOf(status) > -1) {
      ...
      console.log("Valores atuais do ticket antes da atualização:", {
        cpc: ticket.cpc,
        mainTemplate: ticket.mainTemplate,
      });

      if (ticket.cpc === true || ticket.mainTemplate === true || ticket.retryTemplate === true || ticket.lastTemplate === true) {
        await ticket.update({
          cpc: false,
          mainTemplate: false,
          retryTemplate: false,
        ...
        await ticket.reload();
        console.log("Valores atualizados do ticket:", {
          cpc: ticket.cpc,
          mainTemplate: ticket.mainTemplate,
        });
      }

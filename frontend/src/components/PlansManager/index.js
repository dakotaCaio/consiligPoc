            ...
            {(values) => (
                <Form className={classes.fullWidth}>
                    <Grid spacing={1} justifyContent="flex-start" container>
                        ...
                        <Grid xs={12} sm={8} md={2} item>
                            <FormControl margin="dense" variant="outlined" fullWidth>
                                <InputLabel htmlFor="useTemplate-selection">Templates</InputLabel>
                                <Field
                                    as={Select}
                                    id="useTemplate-selection"
                                    label="Templates"
                                    labelId="useTemplate-selection-label"
                                    name="useTemplate"
                                    margin="dense"
                                >
                                    <MenuItem value={true}>{i18n.t("plans.form.enabled")}</MenuItem>
...
export function PlansManagerGrid(props) {
    ...
    const renderTemplates = (row) => {
        return row.useTemplate === false ? `${i18n.t("plans.form.no")}`: `${i18n.t("plans.form.yes")}`;
    }
                ...
                <TableBody>
                    {records.map((row) => (
                        <TableRow key={row.id}>
                            ...
                            <TableCell align="center">{renderTemplates(row)}</TableCell>
                        </TableRow>
                    ))}
                </TableBody>
            </Table>
...
export default function PlansManager() {
    ...
    const handleSelect = (data) => {
        ...
        let useTemplate = data.useTemplate === false ? false : true

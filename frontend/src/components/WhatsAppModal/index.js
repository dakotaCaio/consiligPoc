...
import {
  ...
  FormControlLabel,
  Grid,
  FormControl,    // Correção: Importar FormControl
  InputLabel,     // Correção: Importar InputLabel
  Select,         // Correção: Importar Select
  MenuItem,       // Correção: Importar MenuItem
...
const WhatsAppModal = ({ open, onClose, whatsAppId }) => {
  ...
  const handleSaveWhatsApp = async (values) => {
    ...
    try {
      if (user.companyId !== 1 && user.companyId !== whatsappData.companyId) {
      ...
      toastError(err);
      console.log("Erro ao tentar", err);
    }
};

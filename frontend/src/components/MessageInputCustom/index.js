...
const MessageInputCustom = (props) => {
  ...
  useEffect(() => {
    const fetchPlanConfigs = async () => {
      try {
        ...
        setUseTemplate(planConfigs.plan.useTemplate);
      } catch (err) {
  ...
  const handleOpenTemplatePopup = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:8080/template/${companyId}`, {
          headers: {
  ...
  const handleTemplateSelect = async (template) => {
    if (template.mainTemplate) {
      ...
      console.error("O template não possui o campo 'mainTemplate':", template);
    }
  ...
  if (medias.length > 0)
    ...
    return (
      <Paper square elevation={0} className={classes.mainWrapper}>
        ...
        <Dialog open={openTemplatePopup} onClose={handleCloseTemplatePopup}>
